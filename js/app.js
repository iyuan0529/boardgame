...
function loadScore(){try{svrLoad("score",function(v){if(v&&v.length){scoreRounds=v;scoreCurRound=scoreRounds.length-1;scoreRender()}});var d=localStorage.getItem(scoreKey);if(d)scoreRounds=JSON.parse(d);if(scoreRounds.length>0)scoreCurRound=scoreRounds.length-1;else scoreCurRound=0}catch(e){scoreRounds=[]}}/* ===== 服务端数据持久化 ===== */
...
function savePlayers(){localStorage.setItem(storageKey,JSON.stringify(players));svrSave("players",players);alert("保存成功！")}function startLottery(){if(players.length===0)return alert("请先在「玩家设置」中添加玩家");var btn=document.getElementById("lotBtn");btn.disabled=true;var dom=document.getElementById("lotTxt");dom.classList.add("rolling");var t=setInterval(function(){var r=Math.floor(Math.random()*players.length);dom.innerText=players[r].name;dom.style.color=players[r].color},70);setTimeout(function(){clearInterval(t);dom.classList.remove("rolling");var win=players[Math.floor(Math.random()*players.length)];dom.innerText="起始："+win.name;btn.disabled=false},3000)}
...
function importCustomWords(){var inp=document.getElementById("wordCustomInput").value;var ws=inp.split(",").map(function(w){return w.trim()}).filter(function(w){return w.length>0});if(ws.length===0)return alert("请至少输入一个词语");currentWords=ws;wordIndex=-1;wordVisible=false;document.getElementById("wordText").innerText="点击下方开始";document.getElementById("wordText").classList.add("hidden");updateWordCounter()}
...
window.onload=function(){loadPlayers();initCards();timerSet(30);loadScore();currentWords=WORD_BANKS.animals;wordIndex=-1;go("pageHome");if("serviceWorker"in navigator)navigator.serviceWorker.register("sw.js")};/* ===== 起始玩家 - 全屏炫酷摇号 ===== */
  ...
  (function nextStep() {
    ...
    if (steps < maxSteps) {
      ...
      setTimeout(function() {
        ...
        if (sub) { sub.style.display = 'block'; sub.innerText = '点击屏幕重新摇号'; }
        document.getElementById('lotMsgToggle').style.display='block';
      }, 3000);
    }
  })();
};
